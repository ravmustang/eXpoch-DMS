...
if (DMS_ShowDifficultyColorLegend) then
{
	...
	_title setMarkerColor "ColorGreen";
...

{
	...
	[
		format["DMS_Mission_%1",_x],
		compileFinal preprocessFileLineNumbers (format ["\x\addons\DMS\missions\hero\%1.sqf",_x])
	];

	[_x] call DMS_fnc_SpawnHeroMission;
} forEach DMS_HeroMissionsOnServerStart;
...
for "_i" from 1 to DMS_RandomHeroMissionsOnStart do
{
	[selectRandom DMS_HeroMissionTypesArray] call DMS_fnc_SpawnHeroMission;
};
