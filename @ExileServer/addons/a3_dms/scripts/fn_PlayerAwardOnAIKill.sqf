...
if ((!isNull _playerObj) && {(_playerUID != "") && {_playerType in _unitTypes}}) then
{
	...
	*/
	...
	{
		_written = false;
		if(_AISide isEqualTo "hero")then
		{
			format["addAccountHeroAIKill:%1", getPlayerUID _playerObj] call ExileServer_system_database_query_fireAndForget;
			_written = true;
		};
		if(_AISide isEqualTo "bandit")then
		{
			format["addAccountBanditAIKill:%1", getPlayerUID _playerObj] call ExileServer_system_database_query_fireAndForget;
			_written = true;
		};
		if!(_written)then
		{
			format["addAccountAIKill:%1", getPlayerUID _playerObj] call ExileServer_system_database_query_fireAndForget;
		};
		_newKillerFrags = _playerObj getVariable ["ExileAIKills", 0];
		_newKillerFrags = _newKillerFrags + 1;
		_playerObj setVariable ["ExileAIKills", _newKillerFrags];
		ExileClientPlayerAIKills = _newKillerFrags;
		(owner _playerObj) publicVariableClient "ExileClientPlayerAIKills";
		ExileClientPlayerAIKills = nil;
	};
	...
	if (DMS_Enable_RankChange && {_rankChange!=0}) then
	{
		...
		_playerObj setVariable ["ExileHumanity",_playerRank];
