...
if (DMS_RunningHMissionCount >= DMS_MaxHeroMissions) then
{
	DMS_HMissionLastStart = _time;
};
...
if (diag_fps >= DMS_MinServerFPS && {_playerCount >= DMS_MinPlayerCount}) then
{
	...
	if (DMS_DynamicMission && {_time - DMS_HMissionLastStart > DMS_HMissionDelay}) then
	{
		private _mission = selectRandom DMS_HeroMissionTypesArray;

		if (DMS_DEBUG) then
		{
			(format ["SelectMission :: Selected hero mission: %1",_mission]) call DMS_fnc_DebugLog;
		};

		[_mission] call DMS_fnc_SpawnHeroMission;

		if (DMS_DEBUG) then
		{
			(format ["SelectMission :: Spawning of hero mission ""%1"" complete!",_mission]) call DMS_fnc_DebugLog;
		};
	};
