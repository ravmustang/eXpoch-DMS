...
DMS_HMissionDelay 				= DMS_TimeToFirstMission call DMS_fnc_SelectRandomVal;
...
if (DMS_DEBUG) then
{
	...
	diag_log format ["DMS_DEBUG mission_init :: Random time between missions is initially set to %1s | DMS_HMissionLastStart: %2",DMS_HMissionDelay,DMS_HMissionLastStart];
};
...
DMS_HeroMissionTypesArray = [];
{
	...
	for "_i" from 1 to (_x select 1) do
	{
		DMS_HeroMissionTypesArray pushBack _missionName;
	};

	missionNamespace setVariable
	[
		format["DMS_Mission_%1",_missionName],
		compileFinal preprocessFileLineNumbers (format ["\x\addons\DMS\missions\hero\%1.sqf",_missionName])
	];
} forEach DMS_HeroMissionTypes;
